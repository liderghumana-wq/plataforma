...
import { Play, CheckCircle2, XCircle, ShieldCheck, RefreshCw, AlertTriangle, ChevronDown, ChevronRight, Database, FileSpreadsheet, PieChart } from 'lucide-react';
import { runPrompt24TestSuite, Prompt24TestSuiteSummary, Prompt24TestResult } from '../prompt24TestRunner';

export function Prompt24TestRunnerPanel() {
  const [summary, setSummary] = useState<Prompt24TestSuiteSummary | null>(null);
  ...
  const handleRunTests = () => {
    setIsRunning(true);
    setTimeout(() => {
      const res = runPrompt24TestSuite();
      setSummary(res);
      setIsRunning(false);
    }, 300);
  };
    ...
    <div className="bg-white rounded-3xl border border-slate-200/80 shadow-sm p-6 sm:p-8 space-y-6 text-left text-slate-800">
        ...
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-[10px] font-black uppercase tracking-wider text-emerald-300">
              Suite de Pruebas Automatizadas — Prompt 24
            </span>
          </div>
          <h2 className="text-lg font-black text-white">
            Gestión de Respuestas No Disponibles y Datos Faltantes (14 Pruebas)
          </h2>
          <p className="text-xs text-slate-300 max-w-2xl mt-1 leading-relaxed">
            Verificación estricta de la regla de oro: cero conversión de nulos a "No" o "0", asignación estandarizada de estados de respuesta (ANSWERED, NO, PREFER_NOT_TO_ANSWER, NOT_APPLICABLE, OTHER, MISSING, NOT_ASKED), bases de cálculo reales en indicadores y frases directivas de información insuficiente.
          </p>
        </div>
        ...
        >
          {isRunning ? (
            ...
            <>
              ...
              <span>Ejecutar 14 Pruebas (Prompt 24)</span>
            </>
          )}
        </button>
      </div>
        ...
        <div className="space-y-3 pt-2">
          <div className="flex items-center justify-between border-b border-slate-100 pb-2">
            <h3 className="text-xs font-black text-slate-600 uppercase tracking-wider">
              Resultados de Pruebas Unitarias Prompt 24
            </h3>
                  ...
                  {isExpanded && test.observedOutput && (
                    <div className="px-4 pb-4 border-t border-slate-200/50 pt-3 mt-1">
                      <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider block mb-1">
                        Estructura Observada (Prompt 24 Data Contract)
                      </span>
      ...
      {!summary && !isRunning && (
        <div className="p-8 text-center bg-slate-50 rounded-2xl border border-dashed border-slate-300 space-y-2">
          ...
          <h4 className="text-xs font-black text-slate-800">Suite Lista para Ejecución</h4>
          <p className="text-xs text-slate-500 max-w-md mx-auto">
            Haga clic en el botón superior para verificar los 14 casos obligatorios de gestión de datos no disponibles, preguntas condicionales, importación desde Excel y reportes sin invención de información.
          </p>
        </div>
      )}
    </div>
